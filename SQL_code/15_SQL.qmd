...
SELECT 
  dest, 
  a.name AS dest_name,
  o.Reporting_Airline AS carrier_code, 
...
```{sql, connection = con}
SELECT 
  o.flight_number_reporting_airline AS flight_num,
  a1.name AS dest_name,
  a2.name AS origin_name,
  c.name AS carrier_name
FROM flightdata AS o
...
The result from a LEFT JOIN is always a superset of the result of a (inner, regular, natural) JOIN - every row in the first data set is returned.  Here we find which destinations had no flights on 6/26/19.  

Note that we use CREATE TEMPORARY TABLE to reduce our scope to a single day; if we plan to use all flights on that day in several analyses, it would be much more efficient to first create a temporary table, then run the analyses on that day, and finally drop the temporary table.

```{sql, connection = con}
CREATE TEMPORARY TABLE one_day AS
SELECT *
FROM flightdata 
WHERE Year = 2019 AND Month = 6 AND DayofMonth = 26;
```
...
SELECT 
  ...
  f.origin AS origin
FROM airports AS a 
LEFT JOIN one_day AS f ON a.faa = f.dest
