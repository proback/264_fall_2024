---
title: "MDSR2e Ch 15: Database querying using SQL"
output:
...
#| Note that dim(flights) is reported as NA x 21; there are actually 69 million
#| flights that would take up 20 GB as a csv file!
...
An alternative if the SQL code is stored elsewhere - will only run when knit:

```{sql connection=db, code=readLines("~/R/DS2/15MDSR_SQL.sql"), output.var="test"}
```
...
#   - supposed to show how big data sets can quickly get
n <- 100 * 1e6
...
Treat the table from an initial query as a database for the next query
